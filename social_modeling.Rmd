...
percent_video <- 62.5
percent_search <- 14.28
nodes$p_type <- 'Friend'

video_partcipant <- floor((percent_video/100) * nrow(nodes))

search_partcipant <- floor((percent_search/100) * nrow(nodes))

friend_partcipant <- nrow(nodes) - (video_partcipant+search_partcipant)
...
links$strength <- jitter(links$strength)
```
